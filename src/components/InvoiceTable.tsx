...
export default function InvoiceTable({ invoices, loading, onViewClick }: InvoiceTableProps) {
  ...
  return (
    <div className="space-y-2 w-full">
      ...
      {/* 📱 1. MOBILE RESPONSIVE CARDS VIEW (FIXED FOR CHROMIUM GPU BUG) */}
      <div className="grid grid-cols-1 gap-3 md:hidden transform translate-z-0 will-change-transform">
        {filteredInvoices.length === 0 ? (
          ...
          filteredInvoices.map((inv, index) => (
            /* 🛠️ REMOVED: bg-slate-800/50 and shadow-md -> CHANGED TO: solid bg-slate-800 with no shadow */
            <div key={inv._id} className="bg-slate-800 border border-slate-700 p-4 rounded-xl space-y-3 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center border-b border-slate-700 pb-2">
                ...
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${
                  inv.status === "Active" ? "bg-indigo-950 text-indigo-400 border-indigo-900" : "bg-emerald-950 text-emerald-400 border-emerald-900"
                }`}>
                  {inv.status}
                </span>
              </div>
              ...
              {/* 🛠️ REMOVED: bg-slate-900/40 transparency -> CHANGED TO: solid bg-slate-900 */}
              ...
              <div className="flex items-center justify-between pt-1 border-t border-slate-700 mt-1">
                ...
                <button 
                  ...
                  className="text-[11px] font-medium text-indigo-400 bg-slate-700 hover:bg-slate-600 px-2.5 py-1 rounded-lg border border-slate-600 transition flex items-center gap-1"
                >
      ...
      {/* 🖥️ 2. DESKTOP VIEW TABLE (Replaced opacity/blur with clean slate theme) */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-700 text-sm text-slate-200">
              {filteredInvoices.map((inv, index) => (
                <tr key={inv._id} className="hover:bg-slate-700/40 transition duration-150">
                  ...
                  <td className="p-2 text-right pr-4">
                    <button
                      onClick={() => onViewClick(inv)}
                      className="text-xs text-indigo-400 bg-slate-700 hover:bg-slate-600 p-1 rounded-md border border-slate-600 transition inline-flex items-center gap-1 cursor-pointer"
                    >
