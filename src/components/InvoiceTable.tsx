...
export default function InvoiceTable({
  ...
  const filteredInvoices = invoices.filter((inv) => {
    ...
    const matchesProduct = inv.products?.some((p: any) =>
    ...
    return (
      <div className="w-full bg-slate-800/30 border border-slate-800 rounded-2xl p-8 md:p-12 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs md:text-sm text-slate-400">
          Invoices records load ho rahe hain...
        </p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 gap-3 md:hidden">
        {filteredInvoices.length === 0 ? (
          ...
          filteredInvoices.map((inv, index) => (
            <div
              key={inv._id}
              className="bg-slate-800/50 border border-slate-800/80 p-4 rounded-xl space-y-3 shadow-md relative overflow-hidden [transform:translateZ(0)] backface-visibility-hidden [will-change:transform]"
            >
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-col gap-1 pl-5 text-[11px] text-slate-400">
                  {inv.products?.map((p: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex flex-col bg-slate-900/30 p-1 rounded border border-slate-800/40 mt-0.5"
                    >
              ...
              <div className="grid grid-cols-2 gap-2 bg-slate-900/40 p-2.5 rounded-lg border border-slate-800 text-[11px]">
                <div>
                  ...
                  <p className="text-slate-200 mt-0.5">
                    Rs. {(inv.salePrice || 0).toLocaleString()}
                  </p>
                </div>
                <div>
                  <p className="text-slate-500 text-[10px] font-medium uppercase">
                  ...
                  <p className="text-indigo-300 mt-0.5">
                    {inv.durationMonths === 0
                      ? "Rs. 0"
                      : `Rs. ${(inv.monthlyInstallment || 0).toLocaleString()}/Mo`}
                  </p>
                </div>
                <div className="border-t border-slate-800/60 pt-1.5 mt-1">
                  <p className="text-emerald-500 text-[10px] font-medium uppercase">
                    Advance Paid
                  </p>
                ...
                <div className="border-t border-slate-800/60 pt-1.5 mt-1">
                  <p className="text-amber-500 text-[10px] font-medium uppercase">
                    Remaining
                  </p>
              ...
              <div className="flex items-center justify-between pt-1 border-t border-slate-800/40 mt-1">
                ...
                <button
                  ...
                  className="text-[11px] font-medium text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 px-2.5 py-1 rounded-lg border border-indigo-500/10 transition flex items-center gap-1 cursor-pointer"
                >
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800 text-sm text-slate-200">
              {filteredInvoices.map((inv, index) => (
                <tr
                  key={inv._id}
                  className="hover:bg-slate-700/20 transition duration-150"
                >
                  <td className="p-2 font-medium text-slate-400">
                    {index + 1}
                  </td>
                  <td className="p-2 text-indigo-400 uppercase text-xs font-mono">
                    {inv.invoiceNumber}
                  </td>
                  <td className="p-2 space-y-1 max-w-xs">
                    ...
                    <div className="space-y-1 max-h-24 overflow-y-auto">
                      {inv.products?.map((p: any, idx: number) => (
                        <div
                          key={idx}
                          className="flex flex-col bg-slate-900/20 rounded border border-slate-800/30"
                        >
                  ...
                  <td className="p-2 text-slate-200 text-xs font-mono">
                    Rs. {(inv.salePrice || 0).toLocaleString()}
                  </td>
                  ...
                  <td className="p-2 space-y-0.5">
                    <div className="text-slate-200 text-xs font-mono">
                      {inv.durationMonths === 0
                        ? "Rs. 0"
                        : `Rs. ${(inv.monthlyInstallment || 0).toLocaleString()}`}
                    </div>
                  ...
                  <td className="p-2">
                    <span
                      className={`p-1.5 rounded-md text-[10px] border ${
                        inv.status === "Completed"
                          ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                          : inv.status === "Defaulted"
                            ? "bg-red-500/10 text-red-400 border-red-500/20"
                  ...
                  <td className="p-2 text-right pr-4">
                    <button
                      onClick={() => onViewClick(inv)}
                      className="text-xs text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 p-1.5 rounded-md border border-indigo-500/20 transition inline-flex items-center gap-1 cursor-pointer font-medium"
                    >
