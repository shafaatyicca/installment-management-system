      ...
      <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-xl shadow-2xl p-6 print:border-0 print:shadow-none print:w-full print:max-w-none print:bg-white text-slate-100 print:text-black">
        ...
        <div className="flex justify-between items-center mb-6 pb-2 border-b border-slate-800 no-print">
          ...
          <div className="flex gap-2">
            <button
              onClick={handlePrint}
              className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 cursor-pointer"
            >
              <Printer className="w-4 h-4" /> Print
            </button>
            <button
              onClick={onClose}
              className="bg-slate-800 hover:bg-slate-700 text-slate-400 px-2 py-1.5 rounded-lg cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div
          id="print-area"
          className="space-y-4 font-sans bg-slate-900 print:bg-white p-2"
        >
          {/* Shop Header */}
          <div className="text-center border-b border-dashed border-slate-700 print:border-black pb-3">
            <h2 className="text-xl font-bold uppercase tracking-wider print:text-black">
              Digital Store
            </h2>
            <p className="text-[11px] text-slate-400 print:text-gray-600 mt-0.5">
              Main Bazar, Peshawar, Pakistan
            </p>
          ...
          <div className="grid grid-cols-2 gap-y-1.5 text-xs border-b border-slate-800 print:border-black pb-3">
            <div>
              <p className="text-slate-500 print:text-gray-500 font-medium">
                Invoice / Receipt:
              </p>
              <p className="font-mono font-bold text-slate-200 print:text-black">
                {data.invoiceNumber} /{" "}
                <span className="text-indigo-400 print:text-black">
                  {data.receiptNumber}
                </span>
              </p>
            </div>
            <div className="text-right">
              ...
              <p className="font-mono font-medium text-slate-300 print:text-black">
                {data.receivingDate}
              </p>
            </div>
            <div>
          ...
          <div className="space-y-1 text-xs">
            <p className="text-slate-500 print:text-gray-500 font-bold uppercase text-[10px] tracking-wider">
              Dealt Product(s)
            </p>
            <div className="bg-slate-950/40 print:bg-transparent p-2 rounded-lg border border-slate-800/60 print:border-0 print:p-0 space-y-1">
              {data.products.map((p, idx) => (
                <div
                  key={idx}
                  className="flex justify-between text-slate-300 print:text-black font-medium"
                >
                  <span>• {p.name}</span>
                  {p.brand && (
                    <span className="text-slate-500 print:text-gray-500 text-[10px]">
                      ({p.brand})
                    </span>
                  )}
                </div>
          ...
          <div className="border-t border-dashed border-slate-700 print:border-black pt-3 space-y-1.5">
            <div className="flex justify-between items-center text-sm font-bold bg-indigo-500/10 print:bg-gray-100 p-2.5 rounded-xl">
              ...
              <span className="text-emerald-400 print:text-black font-mono text-base">
                Rs. {data.amountReceived.toLocaleString()}
              </span>
            </div>
