...
export default function ProductTable({ products, loading, onEditClick, onSuccess }: ProductTableProps) {
  ...
  if (loading) {
    return (
      <div className="w-full bg-slate-800 border border-slate-800 rounded-2xl p-8 md:p-12 flex flex-col items-center justify-center gap-3">
    ...
    <div className="space-y-2 w-full">
      ...
      <div className="grid grid-cols-1 gap-3 md:hidden">
        {filteredProducts.length === 0 ? (
          <div className="text-center py-8 text-xs text-slate-500 bg-slate-800 rounded-xl border border-slate-800">
            Stock mein koi product nahi mila.
          </div>
        ) : (
          filteredProducts.map((product, index) => (
            <div key={product._id} className="bg-slate-800 border border-slate-800 p-4 rounded-xl space-y-3 ">
              ...
              <div className="flex justify-between items-start gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-md text-[10px] font-medium shrink-0 ${
                  product.stock > 0 ? "bg-emerald-500 text-emerald-400 border border-emerald-500" : "bg-red-500 text-red-400 border border-red-500"
                }`}>
              ...
              <div className="grid grid-cols-2 gap-2 bg-slate-900 p-2.5 rounded-lg border border-slate-800 text-[11px]">
                <div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-700">
                <button 
                  ...
                  className="text-[11px] font-medium text-amber-400 bg-amber-500 hover:bg-amber-500 px-3 py-1.5 rounded-lg border border-amber-500 transition flex items-center gap-1"
                >
                  <Edit className="w-3 h-3" /> Edit
                </button>
                <button 
                  onClick={() => handleDelete(product._id)}
                  className="text-[11px] font-medium text-red-400 bg-red-500 hover:bg-red-500 px-3 py-1.5 rounded-lg border border-red-500 transition flex items-center gap-1"
                >
      ...
      <div className="hidden md:block w-full bg-slate-800 border border-slate-800 backdrop-blur-md rounded-md overflow-hidden shadow-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-700 bg-slate-800 text-slate-400 text-xs uppercase">
            ...
            <tbody className="divide-y divide-slate-800 text-sm text-slate-200">
              {filteredProducts.map((product, index) => (
                <tr key={product._id} className="hover:bg-slate-700 transition duration-150">
                  ...
                  <td className="p-2 text-right space-x-2">
                    <button 
                      onClick={() => onEditClick(product)}
                      className="text-xs text-amber-400 hover:text-amber-300 bg-amber-500 hover:bg-amber-500 px-2.5 py-1.5 rounded-lg border border-amber-500 transition inline-flex items-center gap-1"
                    >
                      <Edit className="w-3 h-3" />
                    </button>
                    <button 
                      onClick={() => handleDelete(product._id)}
                      className="text-xs text-red-400 hover:text-red-300 bg-red-500 hover:bg-red-500 px-2.5 py-1.5 rounded-lg border border-red-500 transition inline-flex items-center gap-1"
                    >
