...
export default function AddInvoiceModal({
  ...
  // 🟢 NEW: Cart ka total cost track karne ke liye alag state
  ...
  // 🟢 FIXED: Saare selected products ka total dynamic plus karne ke liye alag se effect
  useEffect(() => {
  ...
  // 🟢 FIXED: Mouse scroll aur keyboard up/down keys block karne ke liye handler
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2">
      {/* CSS to remove number input arrows/spinners */}
      <style jsx global>{`
        ...
        <form onSubmit={handleSubmit} className="p-3 space-y-3">
          ...
          <div ref={customerRef} className="relative">
            <label className="block text-xs font-medium text-slate-400 mb-1 flex items-center gap-1">
              <User className="w-3.5 h-3.5 text-indigo-400" /> Customer Search &
              Select*
            </label>
          ...
          <div ref={productRef} className="relative">
            <label className="block text-xs font-medium text-slate-400 mb-1 flex items-center gap-1">
              <Box className="w-3.5 h-3.5 text-indigo-400" /> Product Search &
              Select*
            </label>
            <div className="relative">
              <Search className="absolute left-3 top-3 h-4 w-4 text-slate-500" />
                          ...
                          setSelectedCartItems((prev) => {
                            ...
                            if (exists) {
                              if (exists.quantity >= p.stock) {
                                toast.error(
                                  "Stock mein is se zyada maal nahi hai!",
                                );
                                return prev;
                              }
          ...
          {selectedCartItems.length > 0 && (
            <div className="bg-slate-900/50 border border-slate-700/40 rounded-xl p-3 space-y-2">
              {/* 🟢 FIXED: Top text ke right side par sab products ka total price automatic plus ho kar show ho raha hai */}
          ...
          >
            <div>
              ...
              <input
                ...
                onKeyDown={handleNumberInputKeyDown} // 🟢 FIXED: Block arrow keys
                onWheel={(e) => (e.target as HTMLInputElement).blur()} // 🟢 FIXED: Block mouse wheel scrolling
                placeholder={
                  cartTotalCost > 0
            ...
            {saleMode === "Installment" && (
              <>
                ...
                {/* INSTALLMENT CALCULATION MODE SWITCHER */}
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">
                    Calculation Method
                  </label>
          ...
          <div className="bg-slate-900/60 border border-indigo-500/20 rounded-xl p-4 space-y-2">
            ...
            {saleMode === "Cash" ? (
              ...
              <>
                ...
                {/* 🟢 1. BY AMOUNT KA AUTO-ADJUSTMENT NOTE */}
                {calcMode === "byAmount" &&
                ...
                {/* 🟢 2. BY MONTHS KA AUTO-ADJUSTMENT NOTE (Aapki is condition ke liye) */}
                {calcMode === "byMonths" &&
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-slate-700/60">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`px-6 py-2.5 rounded-xl text-white font-medium shadow-lg transition disabled:opacity-50 text-xs cursor-pointer ${saleMode === "Cash" ? "bg-emerald-600 hover:bg-emerald-500 shadow-emerald-600/10" : "bg-indigo-600 hover:bg-indigo-500 shadow-indigo-600/10"}`}
            >
              {loading
                ? "Generating..."
                : saleMode === "Cash"
                  ? "Generate Cash Invoice"
                  : "Generate Invoice & Book"}
            </button>
          </div>
        </form>
      </div>
