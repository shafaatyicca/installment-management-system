      ...
      <div className="relative w-full max-w-2xl bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-y-auto max-h-[90vh] z-10 text-slate-100">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-700">
          <h2 className="text-xl font-bold flex items-center gap-2 text-indigo-400">
            <FilePlus className="w-5 h-5" /> Naya Installment Bill (Invoice) Banayein
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-slate-400 mb-1 flex items-center gap-1">
                <User className="w-3.5 h-3.5 text-indigo-400" /> Customer Select Karein *
              </label>
              <select 
                value={customerId} 
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-slate-400 mb-1 flex items-center gap-1">
                <Box className="w-3.5 h-3.5 text-indigo-400" /> Product Chunein (Stock se automatic 1 minus hoga) *
              </label>
              <select 
                value={productId} 
            ...
            {/* 3. Sale Price */}
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">Sale Price (Deal Qeemat) *</label>
              <input 
                type="number" 
                value={salePrice} 
                onChange={(e) => setSalePrice(e.target.value)}
                placeholder="Rs. Jitne mein sale kar rahe hain" 
                className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-slate-100 focus:outline-none focus:border-indigo-500 transition text-sm" 
              />
            </div>
            ...
            <div>
              ...
              <input 
                ...
                placeholder="Rs. Jo customer ne mauqe par diye" 
                className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-slate-100 focus:outline-none focus:border-indigo-500 transition text-sm" 
              />
            </div>
            ...
            {/* 5. Duration Months */}
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-slate-400 mb-1">Duration (Kitne Mahine ki Kist/Installment?) *</label>
              <input 
                type="number" 
                value={durationMonths} 
                onChange={(e) => setDurationMonths(e.target.value)}
                placeholder="e.g., 10 Months, 12 Months" 
                className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-slate-100 focus:outline-none focus:border-indigo-500 transition text-sm" 
              />
            </div>
          ...
          <div className="bg-slate-900/60 border border-indigo-500/20 rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-slate-400 text-xs">Remaining Amount (Baqi Raqam):</p>
                <p className="text-lg font-bold text-amber-400">Rs. {remainingAmount.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-slate-400 text-xs">Monthly Installment (Mahana Kist):</p>
                <p className="text-lg font-bold text-emerald-400">Rs. {monthlyInstallment.toLocaleString()} / Month</p>
              </div>
            </div>
          </div>
